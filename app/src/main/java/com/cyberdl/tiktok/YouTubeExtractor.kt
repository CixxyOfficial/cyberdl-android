...
object YouTubeExtractor {
     ...
     */
    fun resolveAudio(youtubeUrl: String): TikTokResult {
        return try {
            ensureInit()
    ...
    /**
     * Resolve link YouTube jadi data video: daftar pilihan resolusi yang
     * benar-benar tersedia untuk video ini (progressive stream, sudah
     * termasuk audio, jadi bisa langsung didownload tanpa proses gabung).
     * Fungsi ini blocking - wajib dipanggil dari background thread.
     */
    fun resolveVideo(youtubeUrl: String): TikTokResult {
        return try {
            ensureInit()
            val streamInfo = StreamInfo.getInfo(ServiceList.YouTube, youtubeUrl.trim())

            val videoOptions = streamInfo.videoStreams
                .filter { !it.content.isNullOrBlank() }
                .distinctBy { it.resolution }
                .sortedByDescending { parseResolutionPixels(it.resolution) }
                .map { VideoOption(it.resolution ?: "SD", it.content) }

            if (videoOptions.isEmpty()) {
                return TikTokResult(success = false, error = "Tidak ada stream video yang bisa diunduh langsung untuk video ini.")
            }
            ...
            TikTokResult(
                ...
                title = streamInfo.name ?: "youtube_video",
                author = streamInfo.uploaderName?.let { "@$it" } ?: "@youtube",
                coverUrl = thumbnailUrl,
                likeCount = if (streamInfo.likeCount > 0) streamInfo.likeCount else 0,
                commentCount = 0,
            ...
            TikTokResult(
                success = false,
                error = "Gagal ambil video YouTube: ${e.message ?: "coba link lain atau update aplikasi"}"
            )
        }
    }

    private fun parseResolutionPixels(resolution: String?): Int {
        if (resolution.isNullOrBlank()) return 0
        val digits = resolution.takeWhile { it.isDigit() }
