...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyModeUi() {
        if (mode == DownloadMode.MP3) {
            binding.tvModeTitle.text = "Audio"
            binding.tvTagline.text = "Tempel link TikTok atau YouTube"
            binding.etUrl.hint = "Tempel link TikTok / YouTube..."
        } else {
            binding.tvModeTitle.text = "Video"
            binding.tvTagline.text = "Tempel link TikTok atau YouTube"
    ...
    private fun resolveLink() {
        ...
        val isYouTube = YouTubeExtractor.isYouTubeLink(url)
        val isTikTok = TikTokApi.isTikTokLink(url)

        if (!isYouTube && !isTikTok) {
            setStatus("link belum dikenali (TikTok/YouTube).", R.color.danger)
            return
        }
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val result = withContext(Dispatchers.IO) {
                when {
                    mode == DownloadMode.MP3 && isYouTube -> YouTubeExtractor.resolveAudio(url)
                    mode == DownloadMode.VIDEO && isYouTube -> YouTubeExtractor.resolveVideo(url)
