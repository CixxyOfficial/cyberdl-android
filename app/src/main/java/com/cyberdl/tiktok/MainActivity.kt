...
class MainActivity : AppCompatActivity() {
    ...
    private fun startDownload(fileUrl: String, result: TikTokResult, resolutionLabel: String?) {
        ...
        showDownloadingDialog(actionWord)
    }
    ...
    private fun showDownloadingDialog(actionWord: String) {
        val view = LayoutInflater.from(this).inflate(R.layout.dialog_download_complete, null)
        view.findViewById<View>(R.id.progressSpinner).visibility = View.VISIBLE
        view.findViewById<View>(R.id.successCircle).visibility = View.GONE
        view.findViewById<TextView>(R.id.tvDialogTitle).text = "Sedang Download $actionWord..."
        view.findViewById<TextView>(R.id.tvDialogMessage).text = "Mohon tunggu sebentar"
        view.findViewById<View>(R.id.btnDialogOk).visibility = View.GONE
    ...
    private fun showDownloadCompleteState() {
        val dialog = statusDialog ?: return
        val subFolder = if (format == DownloadMode.MP3) "MP3" else "Video"
        val isTikTok = platform == Platform.TIKTOK

        dialog.findViewById<View>(R.id.progressSpinner)?.visibility = View.GONE
        dialog.findViewById<View>(R.id.successCircle)?.visibility = View.VISIBLE
        dialog.findViewById<TextView>(R.id.tvDialogTitle)?.text = "Download Selesai"
        dialog.findViewById<TextView>(R.id.tvDialogMessage)?.text = "File tersimpan di folder\nDownload/MOKA.DL/$subFolder"

        val okBtn = dialog.findViewById<TextView>(R.id.btnDialogOk)
