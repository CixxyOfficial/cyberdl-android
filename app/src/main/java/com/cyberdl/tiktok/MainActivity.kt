...
class MainActivity : AppCompatActivity() {
    ...
    private val defaultMediaHeightDp = 200
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.mediaContainer.setOnClickListener { onPlayToggleClicked() }
    ...
    private fun showPreview(result: TikTokResult) {
        ...
        // Reset tinggi media container ke default tiap kali ada link baru
        binding.mediaContainer.layoutParams.height = dpToPx(defaultMediaHeightDp)
        binding.mediaContainer.requestLayout()

        val isVideo = format == DownloadMode.VIDEO
        binding.playBadgeFrame.visibility = if (isVideo) View.VISIBLE else View.GONE
        binding.mediaContainer.isClickable = isVideo
        binding.ivPlayToggle.setImageResource(R.drawable.ic_play)
    }

    private fun dpToPx(dp: Int): Int = (dp * resources.displayMetrics.density).toInt()

    // ---------- Preview video langsung (VideoView native, ukuran nyesuain rasio video) ----------

    private fun onPlayToggleClicked() {
        if (format != DownloadMode.VIDEO) return
        val result = lastResult ?: return
    ...
    private fun toggleVideoPreview(url: String) {
        ...
        binding.playBadgeFrame.visibility = View.GONE
        ...
        binding.videoView.setOnPreparedListener { mp ->
            ...
            if (vw > 0 && vh > 0) {
                val containerWidth = binding.mediaContainer.width.takeIf { it > 0 }
                    ?: resources.displayMetrics.widthPixels
                val newHeight = (containerWidth.toFloat() * vh / vw).toInt()
                binding.mediaContainer.layoutParams.height = newHeight
                binding.mediaContainer.requestLayout()
            }
        }
    ...
    private fun stopVideoPlayback() {
        ...
        if (format == DownloadMode.VIDEO) {
            binding.playBadgeFrame.visibility = View.VISIBLE
        }
        binding.ivPlayToggle.setImageResource(R.drawable.ic_play)
        binding.mediaContainer.layoutParams.height = dpToPx(defaultMediaHeightDp)
        binding.mediaContainer.requestLayout()
