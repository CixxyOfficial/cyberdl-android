    ...
    fun resolve(tiktokUrl: String): TikTokResult {
        ...
        return try {
            client.newCall(request).execute().use { response ->
                ...
                if (play.isBlank() && music.isBlank()) {
                    TikTokResult(success = false, error = "Tidak menemukan video pada link ini.")
                } else {
                    TikTokResult(
                        success = true,
                        videoUrl = play.ifBlank { null },
                        musicUrl = music.ifBlank { null },
                        videoOptions = if (play.isNotBlank()) listOf(VideoOption("HD", play)) else emptyList(),
                        title = title,
                        author = "@$authorName",
                        coverUrl = cover,
